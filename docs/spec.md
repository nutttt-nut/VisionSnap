...
**สถานะ implement ล่าสุด (จาก Oasis, ยังไม่ commit/push ตอนรายงาน)**: unit tests + build + diff check ผ่าน, physical E2E ยัง unverified. Blocker ปัจจุบัน: แยก grab/release ไม่ได้ + pinch ล่าสุดหน้าต่างไม่ขยับ. แผนต่อ: verify Accessibility permission → trace state → explicit grab/release state + confirmation → verify drag/snap ก่อน commit.

**แก้ไข (เช็คโค้ดจริงหลัง merge 16 commits จาก origin/main)**: ไม่พบไฟล์/โค้ด gaze-select หรือ eye-tracking ใดๆ ใน repo เลย (`git grep -i "gaze\|eyetrack"` = ว่างเปล่า) — สิ่งที่ push จริงคือ hand-pointer tracking, pinch-to-drag, 4/5-finger workspace gestures (Desktop switch/Mission Control) เท่านั้น "gaze-select" ที่ Oasis พูดถึงอาจยังอยู่ใน 11 ไฟล์ที่ยัง uncommitted ฝั่งเครื่อง Oasis หรือเป็นความเข้าใจคลาดเคลื่อนในการรายงาน — ต้อง verify กับ Oasis อีกทีตอนถามความคืบหน้ารอบหน้า อย่าเชื่อว่ามีอยู่จริงจนกว่าจะเห็นโค้ด
